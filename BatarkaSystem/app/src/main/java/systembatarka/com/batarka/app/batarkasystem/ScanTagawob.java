...
public class ScanTagawob extends AppCompatActivity implements ZXingScannerView.ResultHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_scan_tagawob);
        Button button = (Button) findViewById(R.id.qrScan1);
...
// Write a string when this client loses connection
        ...
        connectedRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot snapshot) {
                ...
                if (connected) {
                    final Cursor cursor;
                    cursor = dbHelperScanedIDs.getOrder(5);
                    try {
                                   ...
                                   // dbHelperScanedIDs.deleteOrder(cursor.getInt(0));
                                    ...
                                    query.addValueEventListener(new com.firebase.client.ValueEventListener() {
                                        ...
                                        public void onDataChange(com.firebase.client.DataSnapshot dataSnapshot) {
                                            ...
                                            try {
                                                isTagawob = Boolean.parseBoolean(dataSnapshot.child(sharedPref.getString("LastWeekDate", "")).child("Nota").getValue().toString());
                                                if (isTagawob) {
                                                    dbHelperScanedIDs.deleteOrder(cursor.getInt(0), 5);
                                                }
                                            }catch (Exception e){
                                                MainActivity.myFirebaseRef.child("Classes").child(myOwnClass).child("m5dumin").child(cursor.getInt(0)+"").child("Points").child(sharedPref.getString("LastWeekDate", "")).child("Nota").setValue(true);
            ...
            if (day == Calendar.FRIDAY) {
                ...
                query.addValueEventListener(new com.firebase.client.ValueEventListener() {
                    ...
                    public void onDataChange(com.firebase.client.DataSnapshot dataSnapshot) {
                        myPoints = Integer.parseInt(dataSnapshot.child("PointsTotal").getValue().toString());
try {
    isTagawob = Boolean.parseBoolean(dataSnapshot.child(sharedPref.getString("LastWeekDate", "")).child("Tagawob").getValue().toString());

}catch (Exception e){
    MainActivity.myFirebaseRef.child("Classes").child(myOwnClass).child("m5dumin").child(rawResult.getText()).child("Points").child(sharedPref.getString("LastWeekDate", "")).child("Tagawob").setValue(true);
    ...
    dbHelperScanedIDs.addOrder(Integer.parseInt(rawResult.getText().toString()),5);
